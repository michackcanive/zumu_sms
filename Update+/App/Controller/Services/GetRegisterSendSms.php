...
abstract class GetRegisterSendSms
{
    ...
    public function getagendas_($token, $page, $pesquisar)
    {
        try {
            $infor = json_decode($this->estruturaJsonGet($this->URL . "getagendas?pesquisar=" . $pesquisar . "&page=" . $page . "&token=" . $token));
            return $infor;
        } catch (Exception $e) {
            return array(
                'error' => true,
        ...
        try {
            ...
            if (!empty($dataCliente['file']['name'])) {
                ...
                $dados = [
                    ...
                    'message_body' => $dataCliente['message_body'] ?? '',
                    'is_agenda' => $dataCliente['is_agenda'] ?? 0,
                    'start' => $dataCliente['start'] ?? '',
                    'token' => $token
                ];
            }
