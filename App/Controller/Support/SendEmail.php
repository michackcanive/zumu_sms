...
class SendEmail
{
    ...
    {
        $this->para_email = "parceiro@olaservicos.com";
    ...
    public function sendEmailRecupracao($paraOndem, $nomeDoParaOnde, $QuemEstaEnviar, $nomeDeQuemEnvia, $assunto, $corpo)
    {
        ...
        try {
            ...
            $mail->SMTPDebug = SMTP::DEBUG_OFF;                      //Enable verbose debug output
            $mail->isSMTP();                                            //Send using SMTP
            $mail->Host       = 'mail.zumu_sms.com';                     //Set the SMTP server to send through
            $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
            $mail->Username   = 'omnigrej';                     //SMTP username
            $mail->Password   = '#Omin360b!';                               //SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
            $mail->Port       = 465;
            $mail->CharSet = PHPMailer::CHARSET_UTF8;                                  //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
           ...
           // $mail->AddBCC("admin@qlerius.com", "test");
